...
const DOORS = [
  { id: 'HUB',    color: '#6366F1', label: 'S4 HUB',    node: 'NODE // HUB',    title: 'Intelligence & Command',   desc: 'AI briefings, wallet, broker network, cross-door analytics', features: ['SOLVEN AI Daily Brief', 'Unified Wallet & Portfolio', 'Broker B2B Network'], href: '/dashboard', external: false },
  { id: 'EDGE',   color: '#06B6D4', label: 'S4 EDGE',   node: 'NODE // EDGE',   title: 'Trader Platform',          desc: 'MT5 sync, signals, backtesting, trading DNA',                 features: ['MT5 Read-Only Sync', 'Live Signal Feed', 'Trading DNA Profile'], href: 'https://solven4-edge-six.vercel.app', external: true },
  { id: 'FORGE',  color: '#D4A843', label: 'S4 FORGE',  node: 'NODE // FORGE',  title: 'IB Operator',              desc: 'Network management, commissions, channels, operator card',     features: ['IB Network Tree', 'Commission Tracker', 'S4 Channel Command'], href: 'https://solven4-forge-pi.vercel.app', external: true },
  { id: 'ORACLE', color: '#10B981', label: 'S4 ORACLE', node: 'NODE // ORACLE', title: 'Learning Academy',         desc: 'Courses, market analysis, daily briefings, tools',             features: ['Structured Courses', 'Market Analysis Hub', 'AI Learning Tools'], href: 'https://solven4-oracle-eight.vercel.app', external: true },
  { id: 'NEXUS',  color: '#EF4444', label: 'S4 NEXUS',  node: 'NODE // NEXUS',  title: 'Business Command',         desc: 'CRM, leads, automation, social command',                       features: ['Lead CRM Pipeline', 'Social Command Center', 'Workflow Automation'], href: 'https://solven4-nexus-self.vercel.app', external: true },
];
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          ...
          <nav style={{ position: 'sticky', top: 0, zIndex: 50, background: 'rgba(5,5,12,0.82)', backdropFilter: 'blur(20px)', borderBottom: `1px solid ${C.line}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px', height: 60 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 22 }}>
              {['DOORS', 'INTEL', 'MATRIX'].map(x => <span key={x} style={{ ...label(), fontSize: 10.5, cursor: 'pointer' }} className="e-link">{x}</span>)}
          ...
          <section style={{ padding: '70px 24px 30px', maxWidth: 1160, margin: '0 auto' }}>
            <motion.div variants={stagger} initial="hidden" animate="show" style={{ textAlign: 'center' }}>
              <motion.div variants={fadeUp}><Eyebrow>SOLVEN4 INTELLIGENCE PLATFORM</Eyebrow></motion.div>
              ...
              <motion.div variants={fadeUp} style={{ ...label(C.dimmer), fontSize: 11, marginBottom: 30 }}>// AUTH · SUBSCRIPTION · ANALYTICS · CROSS-DOOR CONTROL</motion.div>
              <motion.div variants={fadeUp} style={{ display: 'flex', flexWrap: 'wrap', gap: 14, justifyContent: 'center', marginBottom: 44 }}>
                <button onClick={() => navigate('/auth/login')} style={{ ...btnPrimary, borderRadius: 11, padding: '15px 30px', fontSize: 14, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10 }}>
                  ENTER SOLVEN4 <ArrowRight size={18} />
                </button>
            ...
            <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }}
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                  <span style={label(C.indigoLt)}>SOLVEN AI // WAVEFORM</span>
                  <span style={{ ...label('#34D399'), fontSize: 9.5, display: 'inline-flex', alignItems: 'center', gap: 5 }}><span className="e-dot" style={{ width: 6, height: 6, borderRadius: '50%', background: '#34D399', boxShadow: '0 0 6px #34D399' }} /> LIVE</span>
                </div>
                <Waveform />
                <div style={{ display: 'flex', gap: 18, marginTop: 10 }}>
                  <span style={label()}>SIGNAL / NOISE <b style={{ color: C.cyan }}>4.82</b></span>
                  <span style={label()}>CONFIDENCE <b style={{ color: C.indigoLt }}>82%</b></span>
                </div>
              </div>
              <div>
                <div style={{ ...label(C.indigoLt), marginBottom: 10 }}>// DAILY BRIEF</div>
          ...
          <section style={{ padding: '80px 24px', maxWidth: 1160, margin: '0 auto' }}>
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} style={{ textAlign: 'center', marginBottom: 44 }}>
              ...
              <h2 style={{ fontFamily: C.serif, fontWeight: 900, fontSize: 'clamp(26px, 4vw, 46px)', color: C.ink, margin: 0 }}>THE FIVE DOORS OF SOLVEN4</h2>
              <p style={{ color: C.dim, fontSize: 15, marginTop: 12 }}>A unified intelligence layer built from the ground up for serious MENA traders.</p>
            </motion.div>
                  ...
                  <div className="e-card" style={{ background: C.panel, border: `1px solid ${door.color}33`, boxShadow: `0 0 40px ${door.color}10`, borderRadius: 16, padding: 22, height: '100%', display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
                      ...
                      <div>
                        ...
                        <div style={{ color: C.ink, fontWeight: 600, fontSize: 14 }}>{door.title}</div>
                      </div>
                    </div>
                    <p style={{ color: C.dim, fontSize: 13.5, margin: '0 0 16px', flex: 1 }}>{door.desc}</p>
                    <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 16px', display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {door.features.map(f => (
                        <li key={f} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 13, color: C.dim, borderBottom: `1px solid ${C.line}`, paddingBottom: 8 }}>
                          <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ width: 5, height: 5, borderRadius: '50%', background: door.color }} />{f}</span>
                          <span style={{ ...label(door.color), fontSize: 8.5 }}>v4.0</span>
                        </li>
                      ))}
                    </ul>
                    <div style={{ ...label('#34D399'), fontSize: 9, marginBottom: 12 }}>STATUS · ONLINE · SECURE</div>
                    <a href={door.href} target={door.external ? '_blank' : '_self'} rel="noreferrer" className="e-link"
                      ...
                      ENTER {door.label} <ArrowRight size={13} />
                    </a>
                  </div>
                </motion.div>
              ))}
          ...
          <section style={{ padding: '70px 24px', background: 'rgba(10,12,30,0.5)', borderTop: `1px solid ${C.line}`, borderBottom: `1px solid ${C.line}` }}>
            <div style={{ maxWidth: 1100, margin: '0 auto' }}>
              <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} style={{ textAlign: 'center', marginBottom: 40 }}>
                <Eyebrow color={C.cyan}>WHAT MAKES SOLVEN4 DIFFERENT</Eyebrow>
                <h2 style={{ fontFamily: C.serif, fontWeight: 900, fontSize: 'clamp(24px, 3.6vw, 40px)', color: C.ink, margin: 0 }}>THE INTELLIGENCE MATRIX</h2>
              </motion.div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16 }}>
                {FEATURES.map(({ Icon, title, desc }, i) => (
                  <motion.div key={title} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.07 }}
                    ...
                    <h3 style={{ fontFamily: C.serif, color: C.ink, fontSize: 13.5, fontWeight: 700, letterSpacing: '0.08em', margin: '0 0 8px' }}>{title}</h3>
                    <p style={{ color: C.dim, fontSize: 13.5, margin: 0 }}>{desc}</p>
                  </motion.div>
                ))}
              </div>
            </div>
          ...
          <section style={{ padding: '90px 24px', textAlign: 'center' }}>
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
              <Eyebrow>READY TO ENTER?</Eyebrow>
              <h2 style={{ fontFamily: C.serif, fontWeight: 900, fontSize: 'clamp(30px, 5vw, 58px)', color: C.ink, margin: '0 0 8px', textShadow: '0 0 50px rgba(99,102,241,0.4)' }}>JOIN THE NEXT GENERATION</h2>
              <p style={{ color: C.dim, fontSize: 16, marginBottom: 30 }}>Of MENA traders operating on the intelligence layer.</p>
          ...
          <footer style={{ borderTop: `1px solid ${C.line}`, padding: '26px 24px', maxWidth: 1160, margin: '0 auto', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 14 }}>
            <span style={label()}>© 2026 SOLVEN4 // INTELLIGENCE.HUB // ALL SYSTEMS NOMINAL</span>
            <div style={{ display: 'flex', gap: 20 }}>
              {['PRIVACY', 'TERMS', 'SECURITY'].map(x => <a key={x} href="/legal" style={{ ...label(), fontSize: 10, textDecoration: 'none' }} className="e-link">{x}</a>)}
            </div>
          </footer>
        </div>
      </div>
